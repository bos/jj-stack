...
def test_sync_dry_run_previews_rebase_and_skips_submit_preview(
    ...
    assert exit_code == 0
    assert "Would remove landed prefix" in captured.out
...
def test_sync_reports_nothing_to_submit_when_whole_stack_merged(
    ...
    assert "Nothing to submit: everything on the selected path has landed." in captured.out
...
def test_sync_completes_the_protected_trunk_flow_after_land_via_merge(
    ...
    assert "No landed changes on the selected path need rebasing." in captured.out
...
def test_sync_repairs_one_sibling_path_without_retiring_shared_landed_state(
    ...
    assert "rewritten merge result needs selected recovery" in global_recovery.err
    ...
    assert landed.change_id in ReviewStateStore.for_repo(repo).load().review_identities
    assert "dependent paths remain" in first_sync.out
...
def test_sync_rejects_a_reviewed_unreviewed_reviewed_sandwich_before_mutation(
    ...
    assert exit_code == 1
    assert "reviewed/unreviewed/reviewed sandwich" in captured.err
...
def test_sync_rejects_an_unselected_merge_descendant_before_rebase(
    ...
    assert exit_code == 1
    assert "dependent revisions outside this path" in captured.err
    jj = JjClient(repo)
    ...
    captured = capsys.readouterr()
    assert "repair non" in captured.out
